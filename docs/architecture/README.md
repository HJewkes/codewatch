...
Self-contained interactive HTML maps of the codewatch codebase, generated by `codewatch graph index` + `codewatch graph render` (single snapshot) and `codewatch graph render-diff` (two snapshots, colored). Open with file:// — no backend, no CDN, no install.

> For the **dashboard's** embedded package dependency graph (ELK layered layout, orthogonal edge routing, source-colored fan-in/out) — a different rendering path from the file maps below — see [`dependency-graph-rendering.md`](dependency-graph-rendering.md), which documents its pipeline, capabilities, and limitations.
...
| [`core.html`](core.html) | `@codewatch/core` substrate alone, with the same overlays. | 51 | 51 |
...
```bash
codewatch graph top --db packages/.codewatch/graph.db \
...
codewatch graph relevant --db packages/.codewatch/graph.db \
...
codewatch graph coupled --db packages/.codewatch/graph.db \
...
```bash
codewatch graph report --db packages/.codewatch/graph.db \
  --repo-root packages \
...
Drift comparisons assume both snapshots were indexed by the same `codewatch` version. Comparing across a metrics change (e.g., before/after the addition of `cognitive_max` or `-M`-aware churn) will produce misleading deltas — re-index the baseline if needed. `graph report --vs` and `graph check --baseline` will emit a stderr warning when the two snapshots' index versions differ.

As of indexer 0.2.0, node IDs are rooted at the git toplevel regardless of where you invoke the indexer from — `codewatch graph index ./packages` and `codewatch graph index .` produce the same IDs for files they both cover. Snapshots produced by 0.1.0 used scan-root-relative IDs and are not directly comparable; the version warning catches this case.
