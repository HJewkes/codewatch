...
describe("computeHealth", () => {
  it("is 100 with a clean repo and every component reads 0", () => {
    const { health, healthBreakdown } = computeHealth({
      scary: 0, newViolations: 0, carryViolations: 0, silos: 0, maxComplexity: 10, hiddenCoupling: 0,
    });
  ...
  it("does NOT double-count a scary hotspot that is also a fitness violation", () => {
    ...
    const scaryOnly = computeHealth({
      scary: 2, newViolations: 0, carryViolations: 0, silos: 0, maxComplexity: 10, hiddenCoupling: 0,
    });
  ...
  it("caps each component and floors the score at 0", () => {
    const { health, healthBreakdown } = computeHealth({
      scary: 99, newViolations: 99, carryViolations: 99, silos: 99, maxComplexity: 999, hiddenCoupling: 99,
    });
    expect(healthBreakdown.map((c) => c.penalty)).toEqual([30, 20, 15, 15, 10]);
    expect(health).toBe(10); // 100 - 90
  });
});
